...
//请补充代码
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			for (k = 0; k < 4; k++) {
				//计算出每个小乘积
				num[k] = GFMul(deColM[i][k], origin[k][j]);
			}
			//将所有小乘积异或
			array[i][j] = num[0] ^ num[1] ^ num[2] ^ num[3];
		}
	}
}
...
//请补充代码
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			for (k = 0; k < 4; k++) {
				//计算出每个小乘积
				num[k] = GFMul(colM[i][k], origin[k][j]);
			}
			//将所有小乘积异或
			array[i][j] = num[0] ^ num[1] ^ num[2] ^ num[3];
		}
...
void addRoundKey(int array[4][4], int round) {
	//请补充代码
	//按字节将处理后的明文分组与当前轮次的密钥进行异或
	//把当前轮数对应的w分解成4*4的数组，再与明文进行异或
	int round_key[4];
	int key_matrix[4][4];
	int i = 0;
	int j = 0;
	for (i = 0; i < 4; i++) {
		splitIntToArray(w[round * 4 + i], round_key);
		for (j = 0; j < 4; j++) {
			key_matrix[i][j] = round_key[j];
		}
	}

	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			array[i][j] = array[i][j] ^ key_matrix[i][j];
		}
	}
}
