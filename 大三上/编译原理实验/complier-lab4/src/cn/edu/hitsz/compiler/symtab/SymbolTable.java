...
public class SymbolTable {
    ...
    /**创建HashMap,key为符号的文本表示,value为符号表条目*/
    HashMap<String,SymbolTableEntry> symbolTableMap = new HashMap<String,SymbolTableEntry>();
    ...
    public SymbolTableEntry get(String text) {
        if(symbolTableMap.containsKey(text)){
    ...
    public SymbolTableEntry add(String text) {
        //当该符号已在表中存在，抛出异常RuntimeException
        if(symbolTableMap.containsKey(text)){
        ...
        //加入HashMap
        symbolTableMap.put(text,newEntry);
        //返回新条目
        return newEntry;
        //throw new NotImplementedException();
    }
