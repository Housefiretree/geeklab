...
static int demo_mount(){
    ...
    /* 填充super信息 */
    //通过ddriver_ioctl来获取IO块大小的信息,对应命令号为IOC_REQ_DEVICE_IO_SZ
    ddriver_ioctl(super.driver_fd,IOC_REQ_DEVICE_IO_SZ,&super.sz_io);
    //通过ddriver_ioctl来获取磁盘容量的信息,对应命令号为IOC_REQ_DEVICE_SIZE
    ddriver_ioctl(super.driver_fd,IOC_REQ_DEVICE_SIZE,&super.sz_disk);
    //文件系统demo的一个逻辑块是两个IO块大小
...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    char *buffer=(char *)malloc(super.sz_io*sizeof(char));
    ...
    memcpy(dentry.fname,buffer,sizeof(dentry));
    /* TODO: 填充filename */
